function Generate-Padlock-Token {
    ...
    Write-Host "`n`nUnencrypted padlock token (goes to Configuration record in F&E application): "
    Write-Host -ForegroundColor Green $password

    Write-Host "`nHashed padlock token (goes to Azure ARM template parameters): "
    Write-Host -ForegroundColor Green "$hashedPassword`n"
}

Generate-Padlock-Token