...
namespace FundraisingandEngagement.Services.Xrm
{
    public sealed class XrmService : IXrmService
    {
        ...
        public XrmService(IOptions<CrmOptions> options)
        {
            ...
            this.client = new WebApi(authentication, AppendDynamicsApiVersion(o.CrmOrganizationURL));
        }
        ...
        // By default, the Dynamics API used is version v8.2,
        // which caused the problem with bug #3766. v9.0 fixes the problem.
        private string AppendDynamicsApiVersion(string apiUrl)
        {
            const string dynamicsVersion = "api/data/v9.0/";
            if (!apiUrl.EndsWith("/"))
                apiUrl += "/";
            apiUrl += dynamicsVersion;
            return apiUrl;
        }
    }
}