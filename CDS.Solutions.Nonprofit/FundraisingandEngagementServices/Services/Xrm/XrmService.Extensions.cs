    ...
    public static class XrmServiceExtensions
    {
        ...
        private static Entity ConvertToCrmEntity<T>(T model)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (property.Name == nameof(IContactPaymentEntity.CustomerId))
                {
                    var entityReferenceMap = property.GetEntityReferenceMap();

                    if (model is IContactPaymentEntity customer && customer.CustomerId.HasValue && !String.IsNullOrEmpty(entityReferenceMap))
                    {
                        if (customer.CustomerIdType == CustomerIdType.Account)
                        {
                            entity[$"{entityReferenceMap}_account"] = new EntityReference("account", customer.CustomerId.Value);
                        }
                        else if (customer.CustomerIdType == CustomerIdType.Contact)
                        {
                ...
                else
                {
                    ...
                    if (entityMap != null && !String.IsNullOrEmpty(entityMapName) && entityMap.PushToDataverse)
                    {
                    ...
                    var optionSet = property.GetCustomAttribute<EntityOptionSetMap>();
                    var optionSetName = optionSet?.EntityOptionSet.ToLower();
                    if (optionSet != null && !String.IsNullOrEmpty(optionSetName) && optionSet.PushToDataverse)
                    {
                    ...
                    var referenceMap = property.GetCustomAttribute<EntityReferenceMap>();
                    var refName = referenceMap?.EntityReference;
                    var logicalName = property.GetReferenceLogicalName();
                    if (!String.IsNullOrEmpty(refName) && !String.IsNullOrEmpty(logicalName) && propValue is Guid id && referenceMap != null && referenceMap.PushToDataverse)
                    {
