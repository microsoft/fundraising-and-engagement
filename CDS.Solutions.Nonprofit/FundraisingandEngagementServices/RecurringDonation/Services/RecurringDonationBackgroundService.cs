...
namespace FundraisingandEngagement.RecurringDonations.Services
{
    public class RecurringDonationBackgroundService : BackgroundService
    {
        ...
        private readonly IDbEntitySynchronizer dbEntitySynchronizer;

        private static readonly List<Type> RequiredEntities = new List<Type>
        {
            typeof(Transaction),
            typeof(Response),
            typeof(TransactionCurrency),
        ...
        public RecurringDonationBackgroundService(
                ...
                ILogger logger,
                IDbEntitySynchronizer dbEntitySynchronizer)
        {
            this.repoService = repoService;
            this.paymentProcessorGateway = paymentProcessorGateway;
            this.hostLifetime = hostLifetime;
            this.logger = logger;
            this.dbEntitySynchronizer = dbEntitySynchronizer;
        }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            try
            {
                this.dbEntitySynchronizer.SynchronizeEntitiesToDbTransitively(RequiredEntities);
            }
            catch (Exception e)
            {
                this.logger.LogCritical(e, "Critical error during synchronization from Dataverse");
                exitCode = 1;
            }
