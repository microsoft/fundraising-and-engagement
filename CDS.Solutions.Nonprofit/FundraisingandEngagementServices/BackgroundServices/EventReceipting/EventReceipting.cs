...
namespace FundraisingandEngagement
{
    internal class EventReceipting
    {
        ...
        internal static void UpdateSponsorshipsFromEventSponsorship(EventSponsorship eventSponsorship, PaymentContext context)
        {
        ...
        internal static List<EventPackage> GetEventPackagesFromEventTicket(EventTicket eventTicket, IPaymentContext context)
        {
            ...
            foreach (var curTicket in tickets)
            {
                ...
                {
                    var curEventPackages = context.EventPackage.Where(e => e.EventPackageId == curTicket.EventPackageId && e.StateCode == 0).Distinct().ToList();
        ...
        internal static List<EventPackage> GetEventPackagesFromEventProduct(EventProduct eventProduct, IPaymentContext context)
        {
            ...
            foreach (var curProduct in products)
            {
                ...
                {
                    var curEventPackages = context.EventPackage.Where(e => e.EventPackageId == curProduct.EventPackageId && e.StateCode == 0).Distinct().ToList();
                    eventPackages.AddRange(curEventPackages);
                }
            }
            return eventPackages;
        ...
        internal static List<EventPackage> GetEventPackagesFromEventSponsorship(EventSponsorship eventSponsorship, IPaymentContext context)
        {
            ...
            foreach (var curSponsorship in sponsorships)
            {
                ...
                {
                    var curEventPackages = context.EventPackage.Where(e => e.EventPackageId == curSponsorship.EventPackageId && e.StateCode == 0).Distinct().ToList();
                    eventPackages.AddRange(curEventPackages);
                }
            }
            return eventPackages;
        ...
        internal static void UpdateEventPackage(EventPackage eventPackage, PaymentContext context)
        {
            ...
            var sponsorships = context.Sponsorship.Where(s => s.EventPackageId == eventPackage.EventPackageId);
            ...
            var ticketNonReceiptable = tickets.Sum(t => t.AmountNonreceiptable ?? 0);
            var productNonReceiptable = products.Sum(p => p.AmountNonreceiptable ?? 0);
            var sponsorshipNonpReceiptable = sponsorships.Sum(s => s.AmountNonreceiptable ?? 0);
            ...
            var totalNonReceiptable = ticketNonReceiptable + productNonReceiptable + sponsorshipNonpReceiptable;
