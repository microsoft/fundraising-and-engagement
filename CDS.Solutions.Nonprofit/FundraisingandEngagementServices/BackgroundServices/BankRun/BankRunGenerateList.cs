            ...
            try
            {
                ...
                foreach (PaymentSchedule ps in paymentSchedules)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        logger.LogError(e, "Issue with adding new bank run schedule record for payment schedule " + ps.Name + " (" + ps.PaymentScheduleId + "): " + e.Message);
                    }
                }
                ...
                {
                    // It may already exist, so ignore and move on.
                    logger.LogError(e, "Issue saving Bank Run Schedules: " + e.Message);
                }
            }
            catch (Exception ex)
            {
