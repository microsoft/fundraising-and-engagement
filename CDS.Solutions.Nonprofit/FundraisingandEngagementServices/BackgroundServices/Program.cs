    ...
    public static class Program
    {
        ...
        private static async Task RanCommand(string[] args, ILogger? log = null, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                log?.LogInformation($"Exiting {args[0]}");
            }
            catch (Exception e)
            {
                log?.LogError(e, "Error occurred for invocation with args '{args}'", String.Join(" ", args));
        ...
        private static Command BankRunCommand()
        {
            ...
            command.Handler = CommandHandler.Create<IHost, string, Guid, CancellationToken>((host, action, id, token) => host.Services.GetRequiredService<BankRunOperations>().ExecuteBankRunOperation(action, id));
            return command;
        }
        ...
        private static Command EventReceiptingCommand()
        {
            ...
            command.Handler = CommandHandler.Create<IHost, string, Guid, CancellationToken>((host, entityName, entityId, token) => host.Services.GetRequiredService<EventReceiptingOperations>().ExecuteEventReceipting(entityId, entityName));
            return command;
        }
